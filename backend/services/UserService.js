...
class UserService {
  ...
  async loadSessions () {
    try {
      ...
      console.log(
        `🔐 Loaded ${this.sessions.size} active sessions from persistent storage`
      )
    } catch (error) {
  ...
  async saveSessions () {
    ...
    console.log(
      `🔐 Saved ${sessionsArray.length} sessions to persistent storage`
    )
  }
  ...
  async logout (token) {
    ...
    if (session) {
      ...
      await this.saveSessions()
      console.log(
        `🚪 User logged out: ${session.username} (session: ${session.sessionId})`
      )
    }
  }
  ...
  async deleteUser (userId) {
    ...
    if (deleted) {
      ...
      const userSessions = Array.from(this.sessions.entries()).filter(
        ([token, session]) => session.userId === userId
      )
      ...
      console.log(
        `🗑️ User deleted: ${user.username} (${userSessions.length} sessions removed)`
      )
    }
    return deleted
  }
  ...
  async toggleUserAdmin (userId) {
    ...
    console.log(
      `👑 Admin status toggled for user: ${user.username} (isAdmin: ${user.isAdmin})`
    )
    return user.toJSON()
  }
