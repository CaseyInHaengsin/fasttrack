...
app.post('/api/supplements/:userId', async (req, res) => {
  try {
    ...
    supplements.sort((a, b) => new Date(b.time).getTime() - new Date(a.time).getTime());
...
app.post('/api/import/:userId', async (req, res) => {
  try {
    ...
    if (supplements && supplements.length > 0) {
      ...
      allSupplements.sort((a, b) => new Date(b.time).getTime() - new Date(a.time).getTime());
...
const startServer = async () => {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`💊 Supplement tracking enabled`);
  });
};
